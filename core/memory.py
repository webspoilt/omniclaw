...
class VectorMemory:
    ...
    async def search(self, query: str, limit: int = 5, memory_type: str | None = None) -> list[dict]:
        """
    ...
    def _faiss_search(self, query_embedding: np.ndarray, limit: int) -> list[dict]:
        """Search using FAISS"""
        distances, indices = self.index.search(query_embedding.reshape(1, -1).astype('float32'), limit)
    ...
    def _get_memory_item(self, key: str) -> dict | None:
    ...
    async def find_similar_decompositions(self, goal: str) -> dict | None:
    ...
    async def get_context(self, query: str, conversation_id: str | None = None, limit: int = 5) -> dict[str, Any]:
        """
